...
class PairLoader(Dataset):
    def __init__(self, root_dir, mode, size=256, edge_decay=0, data_augment=True, cache_memory=False):
        ...
        self.img_names = sorted(os.listdir(os.path.join(self.root_dir, 'HQ_syn_new')))
        ...
        t_file_path = os.path.join(self.root_dir, 'LQ_syn_new/haze_coef.txt')
    ...
    def __getitem__(self, idx):
        ...
        t_value = np.float(self.t_values[img_name[:-4]])

        # read images
        if img_name not in self.source_files:
            source_img = read_img(os.path.join(self.root_dir, 'LQ_syn_new', img_name), to_float=False)
            target_img = read_img(os.path.join(self.root_dir, 'HQ_syn_new', img_name), to_float=False)
